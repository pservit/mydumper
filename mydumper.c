...
int main(int argc, char *argv[]) {
  ...
  if (!compress_output) {
    m_open_orig=&g_fopen;
    m_close_orig=(void *) &fclose;
    m_write=(void *)&write_file;
    compress_extension=g_strdup("");
  } else {
    m_open_orig=(void *) &gzopen;
    m_close_orig=(void *) &gzclose;
...
GList *get_chunks_for_table(MYSQL *conn, char *database, char *table,
  ...
  if (indexes){
    ...
    if (!field) {
      ...
      while ((row = mysql_fetch_row(indexes))) {
        if (!strcmp(row[1], "0") && (!strcmp(row[3], "1")) && check_field_type(row[4], conn, database, table)) {
    ...
    g_message("Table %s.%s: Use any index %d, field: %s", database, table,  conf->use_any_index ? 1 : 0, field);
  ...
  guint64 estimated_step, nmin, nmax, cutoff, rows;
